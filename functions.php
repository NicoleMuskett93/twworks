...
//format salary
function format_salary(float $job_salary): string
{
    return number_format($job_salary,0);
}
...
add_action('init', function () {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['custom_job_form_nonce']) && wp_verify_nonce($_POST['custom_job_form_nonce'], 'custom_job_form_action')) {
        ...
        $job_salary = isset($_POST['job_salary']) ? filter_var($_POST['job_salary'], FILTER_SANITIZE_NUMBER_INT) : null;
...
//         // Update the last active timestamp in user meta
//         update_user_meta($user_id, 'last_active_time', time());
//     }
// }
// add_action('init', 'track_user_session');

// function get_active_users_by_company($company_term_id)
// {
//     $current_time = time();
...
//             $user_company = get_field('company_name', 'user_' . $user->ID);
//             $user_company_ID = $user_company->term_id;
//             $last_active_time = get_user_meta($user->ID, 'last_active_time', true);

//             //convert last active time to int
//             $last_active_time = intval($last_active_time);


//             if ($user_company_ID == $company_term_id && ($current_time - $last_active_time) <= $timeout) {
//                 $active_users[] = $user;
//             }
//         }
//     }
